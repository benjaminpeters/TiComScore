...
#define TAG "com.benjamin.comscore"

using namespace v8;

static Persistent<Object> bindingCache;

static void TitaniumcomscoreAndroid_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TitaniumcomscoreAndroid.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::TitaniumcomscoreAndroidBindings::lookupGeneratedInit(
...
static void TitaniumcomscoreAndroid_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TitaniumcomscoreAndroid_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TitaniumcomscoreAndroid_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TitaniumcomscoreAndroidBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TitaniumcomscoreAndroidBinding = {
	"com.benjamin.comscore",
	TitaniumcomscoreAndroid_init,
	TitaniumcomscoreAndroid_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_benjamin_comscore_TitaniumcomscoreAndroidBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.benjamin.comscore", &TitaniumcomscoreAndroidBinding);
	titanium::KrollBindings::addExternalLookup(&(::TitaniumcomscoreAndroidBindings::lookupGeneratedInit));
}
